...
def get(admin, clave_admin):
	try:
		...
		driverPlataforma.get("https://admin.siempregana.net")
			...
			for filaUsuarios in tablaUsuarios:
					...
					if nombreUsuario == usuario:
						try:
							...
							db.insertar_registro(id_cajero_int, "megafaraon", "carga", usuario, monto)
							return True, informe
							break
						except Exception as err:
			...
			if nombreUsuario == usuario:
				...
				if int(monto) > int(fichas_del_usuario):
					...
					try:
						...
						# transofrmo id_cajero y id_plataforma de tupla a int
						id_cajero_int = (''.join(map(str, id_cajero[0])))
						db.insertar_registro(id_cajero_int, "megafaraon", "descarga", usuario, monto)
						return True, informe
						break
					except Exception as err:
