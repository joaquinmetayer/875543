    ...
    if tipo == None:
        ...
        try:
            if desicion == "cancelar":
                ...
                if tipo == "CARGA":
                    informe = bp04.cd_bot_plataforma(tipo, admin, clave_admin, usuario, monto)
        ...
        try:
            if desicion == "cancelar":
                if db.cancelar_registro_desc(id):
                    wpEdge.buscar_chat(numero_admin)
                    wpEdge.enviar_mensaje(f"Operacion de descarga *{id}* cancelada.")
                    db.cambiar_estado("estado2", numero)
                    wpEdge.buscar_chat(numero)
