...
def btnNoMic():
    ...
    btnNoMic = WebDriverWait(driver, timeout=5).until(
        lambda d: d.find_element(By.XPATH, xpath)).click()
    cancel = WebDriverWait(driver, timeout=5).until(
...
def enviar_mensaje(mensaje):
    ...
    chatbox = WebDriverWait(driver, timeout=5).until(
...
def leer_ultimo_mensaje():
    element_box_message = WebDriverWait(driver, timeout=5).until(
...
def buscar_chat(nombre_chat): #nombre_chat = nombre si esta agendado, o numero de telefono si no lo esta.
    ...
    barraDeBusqueda = WebDriverWait(driver, timeout=5).until(
        lambda d: d.find_element(By.XPATH, xpath))
