...
numero_admin = numero_superadmin
...
def menu_acciones(numero, usuario):
    ...
    if opc != "1" and opc != "2" and opc != "3" and opc != "4" and opc != "5":
        ...
        if opc == "1":
            ...
            wp.enviar_mensaje("Dame un momento, estoy procesando tu *solicitud de desbloqueo*")
            informe = bp03.cd_bot_plataforma("DESBLOQ", admin, clave_admin, usuario, None)
            if informe == "desbloqueado":
                wp.enviar_mensaje(msjs.msj_user_desbloqueado())
                wp.enviar_mensaje_menu_acciones(usuario)
            elif informe == "no bloqueado":
                wp.enviar_mensaje(msjs.msj_user_no_bloqueado())
                wp.enviar_mensaje_menu_acciones(usuario)
            else:
                wp.enviar_mensaje("Algo salio mal. Volve a intentarlo dentro de un rato. O contactate con Atención al cliente.")
        else:
            # atencion cliente
...
def datos_retiro(numero,usuario):
    ...
    if datosSplit[0].lower() == "cancelar":
        ...
        fichas_reales = bp03.cd_bot_plataforma("FICHAS", admin, clave_admin, usuario, None)
        if int(fichas_reales) < int(datosSplit[0]) or int(datosSplit[0]) == 0:
            ...
            informe = bp03.cd_bot_plataforma("DESCARGA", admin, clave_admin, usuario, datosSplit[0])
...
def confirmacion_pw(numero, usuario):
    ...
    if confirmacion == "confirmar":
        ...
        informe = bp03.cd_bot_plataforma("PASSW", admin, clave_admin, usuario, None)
        if not "ERROR" in informe:
            time.sleep(1)
...
def detectar_comando():
    ...
    elif len(comandoSplit) == 4 and comandoSplit[2].lower() == "recibido" and int(comandoSplit[3]) > 0:
        wp.enviar_mensaje("Ejecutando comando")
        wp.buscar_chat("Leem")
    ...
    # COMANDO PARA CONSULTAR FICHAS EN PANEL DE ADMINISTRADOR
        ...
        wp.enviar_mensaje("Ejecutando comando")
        wp.buscar_chat("Leem")
        fichas = bp03.fichas_administrador()
