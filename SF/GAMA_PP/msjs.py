...
def msj_pasos_transf(alias,cbu,nombre_cuenta):
    msj = "Seguí estos pasos para cargar fichas: *(MINIMO DE CARGA $600)*"
    msj0 ="*# PASO 1*"
...
def msj_err_reglas_user(usuario):
    msj = f"*{usuario}* tiene una longitud de *{len(usuario)}*. Recorda debe contener más de 4 letras y menos de 16. Probemos con otro"
    return msj
...
def msj_datos_retiro(fichas_a_retirar, cbu):
    msj = f"""*Listo*. Retire {fichas_a_retirar} fichas de tu cuenta.
           *Y genere la solicitud de retiro.* Te aviso cuando la transferencia haya sido realizada al cbu: *{cbu}*. ¿Puedo ayudarte en algo más?"""
    return msj
