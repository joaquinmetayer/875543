...
def crear_usuario(numero):
    ...
    elif bp05.botNuevoJugador("arlequin21", "93113seba", usuario, "Hola123"):
...
def datos_retiro(numero,usuario):
    ...
    if datosSplit[0].lower() == "cancelar":
        ...
        fichas_reales = bp05.cd_bot_plataforma("FICHAS", admin, clave_admin, usuario, None)
        if int(fichas_reales) < int(datosSplit[0]) or int(datosSplit[0]) == 0:
            ...
            time.sleep(2)
            informe = bp05.cd_bot_plataforma("DESCARGA", admin, clave_admin, usuario, datosSplit[0])
...
def confirmacion_pw(numero, usuario):
    ...
    if confirmacion == "confirmar":
        ...
        informe = bp05.cd_bot_plataforma("PASSW", admin, clave_admin, usuario, None)
        if not "ERROR" in informe:
            time.sleep(1)
...
def detectar_comando():
    ...
    # COMANDO PARA CONSULTAR FICHAS EN PANEL DE ADMINISTRADOR
        ...
        wp.enviar_mensaje("Ejecutando comando")
        wp.buscar_chat("Leem")
        fichas = bp05.fichas_administrador()
